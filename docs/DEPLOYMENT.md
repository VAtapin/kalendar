...
В `CALENDAR_OWNER_EMAIL` обязательно укажите свой e-mail в нижнем регистре. Только
пользователь, подтвердивший этот адрес ссылкой из письма, сможет создавать,
перезаписывать и удалять общие макеты календарной сетки. Проверка выполняется
сервером; остальные пользователи могут только применять опубликованные макеты.
Общие макеты хранятся в `storage/calendar-grid-templates.json`.
