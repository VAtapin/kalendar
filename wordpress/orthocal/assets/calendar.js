(() => {
  ...
  function iconPopup(icon,t,endpoint) {
    ...
    icons=icons.map(item=>({title:typeof item?.title==='string'&&item.title?item.title:t('Икона'),description:typeof item?.description==='string'?item.description:'',dates:[...new Set((Array.isArray(item?.dates)?item.dates:[]).filter(date=>typeof date==='string'&&date))],images:[...new Set((Array.isArray(item?.images)?item.images:[]).filter(url=>typeof url==='string'&&url))]})).filter(item=>item.images.length);
