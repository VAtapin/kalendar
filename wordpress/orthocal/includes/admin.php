...
final class Orthocal_Admin {
    ...
    static function page() {
        ...
        echo '<header class="oc-admin-hero"><div><span>КАЛЕНДАРНАЯ МАСТЕРСКАЯ · '.esc_html(Orthocal_Plugin::VERSION).'</span><h1>Православный календарь</h1><p>Подключите источники, затем соберите и сразу увидьте каждый блок.</p></div><div class="oc-admin-badge">'.($o['key']||defined('ORTHOCAL_API_KEY')?'Ключ задан':'Нужен API-ключ').'<small>'.$stats['count'].' локальных файлов · '.esc_html(size_format($stats['bytes'])).'</small></div></header>';
        ...
        echo '<section class="oc-admin-card"><h2>Подключение к данным</h2><p>Календарные события и праздники приходят из API календаря. Полные библейские тексты — из BibleDesktop.</p><div class="oc-admin-fields">';
    ...
    static function generator() {
        ...
        self::build_select('mode','Что разместить',Orthocal_Plugin::TITLES,'today');
