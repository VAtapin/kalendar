...
<template><section><h2>Учётные записи</h2><p>Подтверждённые аккаунты. Пароли не доступны администратору; восстановление выполняет сам пользователь через письмо.</p><p v-if="error" role="alert">{{ error }}</p><p v-if="!users.length">Аккаунтов пока нет.</p><article v-for="user in users" :key="user.id"><strong>{{ user.email }}</strong><p>{{ user.blocked ? 'Заблокирован' : 'Активен' }} · {{ user.createdAt }}</p><button :disabled="busy" @click="block(user)">{{ user.blocked ? 'Разблокировать' : 'Заблокировать и завершить сеансы' }}</button></article></section></template>
