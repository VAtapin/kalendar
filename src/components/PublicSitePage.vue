...
<template><main class="public-site-page"><header><a href="/">Календарная мастерская</a><select v-model="interfaceLanguage" aria-label="Язык"><option v-for="option in INTERFACE_LANGUAGE_OPTIONS" :key="option.id" :value="option.id">{{option.nativeLabel}}</option></select></header>
...
<footer><span>Volodymyr Atapin · ATAPIN.DE</span><nav><a v-for="entry in sitePages" :key="entry.id" :href="'/?page='+encodeURIComponent(entry.slug)">{{entry.translations[interfaceLanguage]?.title ?? entry.translations.de?.title ?? entry.slug}}</a></nav></footer></main></template>
