...
<style scoped>
.page-html{overflow-wrap:anywhere}.page-html :deep(img){max-width:100%;height:auto}.page-html :deep(table){max-width:100%;border-collapse:collapse}.page-html :deep(td),.page-html :deep(th){border:1px solid #aaa;padding:6px}
</style>
...
<p v-if="loading">Загрузка…</p><p v-else-if="error" role="alert">{{error}}</p><article v-else-if="translation" data-no-translate><p v-if="fallback">Перевод на выбранный язык ещё не опубликован. Показана доступная версия.</p><h1>{{translation.title}}</h1><div class="page-html" v-html="pageHtml(translation)" /></article><h1 v-else>Страница не найдена</h1>
