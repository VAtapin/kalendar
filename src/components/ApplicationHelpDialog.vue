    ...
    >
      ...
      <div v-if="page === 'guide'" class="application-dialog__content help-guide">
        ...
        <section>
          ...
          <p><strong>Верхнее меню</strong> — создание, открытие, локальное сохранение, совместная ссылка, экспорт, команды макета и справка. Полоса под ним показывает координаты и размер выбранного объекта.</p>
          ...
          <p><strong>Нижняя строка</strong> — ошибки проверки, состояние локального автосохранения, совместной работы и экспорта.</p>
        </section>
        <section>
        ...
        <section>
          ...
          <p>Команда <strong>Файл → Поделиться для совместной работы…</strong> сохраняет копию проекта на сервере и выдаёт ссылку. Один человек редактирует документ, а второй видит, кем он занят, и может ждать освобождения либо сделать независимую копию.</p>
          <p>Открытая вкладка подтверждает работу каждые 15 секунд. После закрытия вкладки доступ освобождается сразу, а при обрыве связи — примерно через 45 секунд.</p>
        </section>
        <section>
        ...
        <section>
          <h3>Печать</h3>
          <p>Перед экспортом откройте панель «Проверка», устраните ошибки и проверьте предупреждения. Команда «Экспортировать печатный PDF…» формирует многостраничный файл с реальными размерами, вылетами и метками реза, частями передаёт его на сервер и показывает ссылку для устойчивого скачивания.</p>
        </section>
      </div>
