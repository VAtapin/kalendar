...
function pushFixedAfterfeasts(year: number, definitions: GeneratedDefinition[]): void {
  for (const feast of FIXED_AFTERFEASTS) {
    for (const julianYear of [year - 1, year]) {
      ...
      // Test the leave-taking date, not the feast date: when Annunciation is
      // on Friday of week 6, the following Lazarus Saturday has no leave-taking.
      // Typikon March 26 rubrics (also excludes Holy/Bright Week).
      // It must not create an ordinary afterfeast on Holy/Bright Week.
      if (feast.julianMonth === 3 && feast.julianDay === 25 &&
        compareDates(addDays(feastDate, 1), addDays(calculateOrthodoxPascha(feastDate.year), -8)) >= 0) {
        continue;
      }
