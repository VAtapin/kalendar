...
const TRANSLATIONS: Record<string, Translation> = {
  ...
  "Параметры числа, старого стиля, текста событий, знаков типикона и поста задаются независимо. Кнопка «Шаблоны календаря» в левой панели открывает заготовки проекта и сетки; готовую сетку можно применить ко всем месяцам.": { de: "Tageszahl, Datum nach altem Stil, Ereignistext sowie Typikon- und Fastenzeichen werden unabhängig eingestellt. „Kalendervorlagen“ links öffnet Projekt- und Rastervorlagen; ein fertiges Raster kann auf alle Monate angewendet werden.", en: "Day number, Old Style date, event text, Typikon marks and fasting icons are configured independently. Calendar Templates opens project and grid presets; a finished grid can be applied to all months.", uk: "Параметри числа, старого стилю, тексту подій, знаків типікону й посту задаються незалежно. Кнопка «Шаблони календаря» ліворуч відкриває заготовки проєкту та сітки; готову сітку можна застосувати до всіх місяців." },
