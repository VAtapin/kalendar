...
try {
  ...
  const outer=page.locator('dialog.oc-icon-summary-dialog');
  assert.equal(await outer.locator('.oc-icon-counter').innerText(),'1 из 3');
  ...
  assert.equal(await outer.locator('.oc-icon-thumbnail').count(),2);
  assert.equal(await outer.locator('.oc-day-icon-choice').count(),3);
  assert.ok((await outer.locator('.oc-icon-thumbnails').boundingBox()).y<(await outer.locator('.oc-icon-description').boundingBox()).y);
  assert.equal(await outer.locator('.oc-icon-large').isHidden(),true);
  await outer.locator('.oc-icon-thumbnail').first().click();
  await outer.locator('.oc-icon-large').waitFor({state:'visible'});
  assert.equal(await outer.locator('.oc-icon-counter').innerText(),'2 из 3');
  assert.equal(await page.locator('dialog.oc-icon-image-dialog').count(),0);
  await outer.press('ArrowRight');
  assert.equal(await outer.locator('.oc-icon-counter').innerText(),'3 из 3');
  assert.match(await outer.locator('h2').innerText(),/Икона Божией Матери Великая/);
  await outer.locator('.oc-day-icon-choice').nth(1).click();
  assert.equal(await outer.locator('.oc-icon-counter').innerText(),'1 из 2');
  assert.match(await outer.locator('.oc-icon-description').innerText(),/Описание второго образа/);
  assert.equal(await outer.locator('.oc-icon-thumbnail').count(),1);
  await outer.locator('.oc-dialog-close').click();
