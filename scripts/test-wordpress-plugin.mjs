...
try {
  ...
  await page.goto(origin+'/wp-admin/options-general.php?page=orthocal');
  assert.equal(await page.locator('input[name="orthocal_options[key]"]').inputValue(),'');
  assert.ok(!(await page.content()).includes('local-test-secret'));
  await page.locator('[data-oc-tab="appearance"]').click();
  await page.locator('select[name="orthocal_options[theme]"]').selectOption('modern');
  await page.locator('select[name="orthocal_options[reading_open]"]').selectOption('modal');
  await Promise.all([page.waitForURL('**settings-updated=true'),page.locator('#submit').click()]);
  assert.equal(await page.locator('select[name="orthocal_options[theme]"]').inputValue(),'modern');
  ...
  assert.equal(await page.locator('[data-oc-help-item]').count(),19);
