...
try {
  ...
  const outer=page.locator('dialog.oc-dialog:not(.oc-icon-image-dialog)');
  ...
  assert.equal(await outer.locator('.oc-icon-large').count(),0);
  await outer.locator('.oc-icon-thumbnail').first().click();
  const imageDialog=page.locator('dialog.oc-icon-image-dialog');
  await imageDialog.locator('.oc-icon-large').waitFor({state:'visible'});
  assert.equal(await imageDialog.locator('.oc-icon-counter').innerText(),'1 из 2');
  assert.equal(await imageDialog.locator('.oc-icon-thumbnail').count(),2);
  await imageDialog.press('ArrowRight');
  assert.equal(await imageDialog.locator('.oc-icon-counter').innerText(),'2 из 2');
  await imageDialog.locator('.oc-dialog-close').click();
  assert.equal(await outer.isVisible(),true);
  await outer.locator('.oc-icon-nav').nth(1).click();
