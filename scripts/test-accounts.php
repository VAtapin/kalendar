...
check_account(calendar_timestamp($trash[0]['deletedAt']) >= $deletedAfter && calendar_timestamp($trash[0]['deletedAt']) <= time(), 'Trash exposes deletion time as UTC');
$oldTrashFile = $record['id'] . '-1760000000.json';
rename($directory . '/calendar-trash/' . $trash[0]['id'], $directory . '/calendar-trash/' . $oldTrashFile);
touch($directory . '/calendar-trash/' . $oldTrashFile, 1500000000);
$trash = $store->accountTrash($a['id']);
check_account($trash[0]['deletedAt'] === '2025-10-09T08:53:20.000Z', 'Existing trash uses filename deletion time, not file mtime or last save');
