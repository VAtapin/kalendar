...
assert.equal(library.status,'published');assert.equal(library.count,39);assert.equal(library.cards.length,8);
...
const page=readFileSync('public/icons-of-mother-of-god.html','utf8');for(const text of ['page:1','lightbox','place','theme','day','/api/v1/icons/mother-of-god'])assert.ok(page.includes(text),text);
