...
try {
  ...
  await admin.waitFor();
  await admin.getByRole('button',{name:'Ресурсы: шрифты, SVG, изображения',exact:true}).click();
  await admin.getByLabel('Название нового ресурса',{exact:true}).fill('Тестовая фотография');
  await admin.getByRole('checkbox').check();
  await admin.locator('input[type=file]').setInputFiles({name:'test.png',mimeType:'image/png',buffer:Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aX1cAAAAASUVORK5CYII=','base64')});
  await admin.getByText('Загружен материал «Тестовая фотография»',{exact:true}).waitFor();
  await admin.getByLabel('Поиск ресурса').fill('Тестовая фотография');
  await admin.getByRole('button',{name:'Изменить / выключить'}).click();
  await admin.getByLabel('Доступен пользователям').uncheck();
  await admin.getByRole('button',{name:'Сохранить',exact:true}).click();
  await admin.getByText('Изменения сохранены',{exact:true}).waitFor();
  await page.screenshot({path:'tmp/admin-catalog.png'});
  await admin.getByRole('button',{name:'Макеты сеток',exact:true}).click();
  await admin.getByRole('button',{name:'Редактировать визуально',exact:true}).first().click();
  ...
  await admin.getByRole('button',{name:'+ Заголовок',exact:true}).first().click();
  await admin.getByRole('button',{name:'+ Изображение',exact:true}).first().click();
  await admin.getByRole('button',{name:'+ Кнопка',exact:true}).first().click();
  await admin.getByText('Поля блоков и ссылки',{exact:true}).click();
  assert.equal(await admin.locator('.newsletter-editor details fieldset').count(),4);
