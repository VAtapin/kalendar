...
try {
  ...
  await admin.waitFor();
  await admin.getByRole('button',{name:'Общие макеты',exact:true}).click();
  ...
  await admin.getByRole('button',{name:'+ Заголовок',exact:true}).click();
  await admin.getByRole('button',{name:'+ Изображение',exact:true}).click();
