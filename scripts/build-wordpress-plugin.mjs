...
if(process.platform==='win32') {
  try { execFileSync('powershell',['-NoProfile','-Command',"Compress-Archive -LiteralPath 'wordpress/orthocal' -DestinationPath 'artifacts/orthocal-1.3.46.zip' -Force"],{stdio:'inherit'}); }
  catch { execFileSync('tar.exe',['-a','-c','-f',resolve('artifacts/orthocal-1.3.46.zip'),'-C',resolve('wordpress'),'orthocal'],{stdio:'inherit'}); }
}
else execFileSync('zip',['-qr',resolve('artifacts/orthocal-1.3.46.zip'),'orthocal'],{cwd:resolve('wordpress'),stdio:'inherit'});
console.log('artifacts/orthocal-1.3.46.zip');
...
if(process.argv.includes('--publish-downloads')){
  ...
  copyFileSync('artifacts/orthocal-1.3.46.zip','public/downloads/orthocal-1.3.46.zip');
