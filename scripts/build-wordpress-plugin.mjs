...
for(const [mode,title] of Object.entries(titles)) {
  ...
  writeFileSync(resolve(directory,'block.json'),JSON.stringify({
    $schema:'https://schemas.wp.org/trunk/block.json',apiVersion:3,name:'orthocal/'+mode,version:'1.3.43',
...
if(process.platform==='win32') {
  try { execFileSync('powershell',['-NoProfile','-Command',"Compress-Archive -LiteralPath 'wordpress/orthocal' -DestinationPath 'artifacts/orthocal-1.3.43.zip' -Force"],{stdio:'inherit'}); }
  catch { execFileSync('tar.exe',['-a','-c','-f',resolve('artifacts/orthocal-1.3.43.zip'),'-C',resolve('wordpress'),'orthocal'],{stdio:'inherit'}); }
}
else execFileSync('zip',['-qr',resolve('artifacts/orthocal-1.3.43.zip'),'orthocal'],{cwd:resolve('wordpress'),stdio:'inherit'});
console.log('artifacts/orthocal-1.3.43.zip');
...
if(process.argv.includes('--publish-downloads')){
  ...
  copyFileSync('artifacts/orthocal-1.3.43.zip','public/downloads/orthocal-1.3.43.zip');
