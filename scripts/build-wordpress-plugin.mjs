...
for(const [mode,title] of Object.entries(titles)) {
  ...
  writeFileSync(resolve(directory,'block.json'),JSON.stringify({
    $schema:'https://schemas.wp.org/trunk/block.json',apiVersion:3,name:'orthocal/'+mode,version:'1.3.1',
    title:'Православный календарь: '+title,category:'widgets',icon:'calendar-alt',textdomain:'orthocal',
    attributes:Object.fromEntries(['date','year','month','limit','filter','lang','profile','theme','css_mode','compact','oldstyle','show_nav','show_picker','show_copy','open','reading_open','images','image_size','image_pack','icons','heading','sections','event_levels','scope','tone','weekday','text_id'].map(name=>[name,{type:'string'}])),
...
if(process.platform==='win32') execFileSync('powershell',['-NoProfile','-Command',"Compress-Archive -LiteralPath 'wordpress/orthocal' -DestinationPath 'artifacts/orthocal-1.3.1.zip' -Force"],{stdio:'inherit'});
else execFileSync('zip',['-qr',resolve('artifacts/orthocal-1.3.1.zip'),'orthocal'],{cwd:resolve('wordpress'),stdio:'inherit'});
console.log('artifacts/orthocal-1.3.1.zip');
...
if(process.argv.includes('--publish-downloads')){
  ...
  copyFileSync('artifacts/orthocal-1.3.1.zip','public/downloads/orthocal-1.3.1.zip');
