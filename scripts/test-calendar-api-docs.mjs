...
try {
  ...
  const pluginLink=page.getByRole('link',{name:`Скачать WordPress-плагин · ${version}`});
  assert.equal(await pluginLink.getAttribute('href'),`/downloads/orthocal-${version}.zip`);
  const zip=await fetch(origin+`/downloads/orthocal-${version}.zip`);assert.equal(zip.status,200);const bytes=new Uint8Array(await zip.arrayBuffer());assert.equal(String.fromCharCode(...bytes.slice(0,2)),'PK');
  const guide=await fetch(origin+'/downloads/calendar-api-guide.md');assert.equal(guide.status,200);const guideText=await guide.text();assert.ok(guideText.includes('X-API-Key'));assert.ok(guideText.includes(`/downloads/orthocal-${version}.zip`));
