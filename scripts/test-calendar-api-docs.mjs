...
try {
  ...
  const zip=await fetch(origin+'/downloads/orthocal-1.3.32.zip');assert.equal(zip.status,200);const bytes=new Uint8Array(await zip.arrayBuffer());assert.equal(String.fromCharCode(...bytes.slice(0,2)),'PK');
